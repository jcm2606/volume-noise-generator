...
fn generate_lf_cloud_noisetex(index: u32) {
    ...
    noisetex.fill(|info, pixel, pos| {
        ...
        let perlin_worley = {
            let perlin: f32 = sample_perlin_fbm(uvw, 5, 12f32, 2f32, &mut seed) * 0.5f32 + 0.5f32;
...
fn generate_perlin_noisetex(index: u32) {
    ...
    noisetex.fill(|info, pixel, pos| {
        ...
        let noise_2 = sample_perlin_fbm_2d(uvw.xy(), 3, 32f32, 2f32, &mut seed) * 0.5f32 + 0.5f32;
        let noise_3 = sample_perlin_fbm_2d(uvw.xy(), 2, 128f32, 2f32, &mut seed) * 0.5f32 + 0.5f32;
...
fn generate_worley_noisetex(index: u32) {
    let mut noisetex = NoisetexRgba8::new(512, 512, 1);
    ...
    noisetex.fill(|info, pixel, pos| {
        ...
        let noise_0 = 1f32 - sample_worley_fbm_2d(uvw.xy(), 3, 4f32, 2f32, &mut seed).0;
...
fn main() {
    ...
    for i in 0..1 {
        generate_perlin_noisetex(i);
        generate_worley_noisetex(i);
    }
